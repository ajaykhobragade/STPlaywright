...
const { LazyLoadOff_URL_P } = require('./urls');
const { LazyLoadOff_URL_S } = require('./urls');
...
(async () => {
  ...
  //#region navigate to the pages
    await page1.goto(LazyLoadOff_URL_P, {
        waitUntil: 'load',
        timeout: 0
      });
  ...
  //#endregion
    ...
    await page2.goto(LazyLoadOff_URL_S, {
        waitUntil: 'load',
        timeout: 0
      });
  ...
  //#region compare ad div counts
    ...
    /* const divIDs = await page1.$$eval('.dfp-ad-container', (divs) => divs.);
    console.log('The total number of Ad div IDs with matching class on prod is ' + divIDs); */
  ...
  //#region capture fullpage screenshot
    await page1.screenshot({ 
      path: `ADV-236_Article_FeatOff-${browsertype}.png`,
      fullPage : true });
