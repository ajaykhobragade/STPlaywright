...
const { LazyLoad_SectionFront_URL_P } = require('./urls');
const { LazyLoad_SectionFront_URL_S } = require('./urls');
...
(async () => {
  ...
  //#region navigate to the pages
    await page1.goto(LazyLoad_SectionFront_URL_P, {
        waitUntil: 'load',
        timeout: 0
      });
  ...
  //#endregion
    ...
    await page2.goto(LazyLoad_SectionFront_URL_S, {
        waitUntil: 'load',
        timeout: 0
      });
  ...
  //#region capture fullpage screenshot
    await page1.screenshot({ 
      path: `Prod-ADV-234_SectionFront_DivCont-${browsertype}.png`,
      fullPage : true });
    console.log('Screenshot captured on ' + browsertype + ' browser!');

    await page2.screenshot({ 
      path: `Stage-ADV-234_SectionFront_DivCont-${browsertype}.png`,
      fullPage : true });
    console.log('Screenshot captured on ' + browsertype + ' browser!');
  //#endregion
